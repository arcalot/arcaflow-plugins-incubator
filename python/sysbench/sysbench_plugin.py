...
class SysbenchCpuInputParams:
    ...
    """
    operation: str = field(
        metadata={"name": "Operation", "description": "Sysbench Operation to perform"})
    threads: typing.Optional[int] = field(default=1, metadata={
                                          "name": "Threads", "description": "Number of worker threads to create"})
    events: typing.Optional[int] = field(default=0, metadata={
                                         "name": "Number of events", "description": "Maximum number of events"})
    cpumaxprime: typing.Optional[int] = field(default=10000, metadata={
                                              "name": "CPU max prime", "description": "The upper limit of the number of prime numbers generated"})
    time: typing.Optional[int] = field(default=10, metadata={
                                       "name": "Time", "description": "Limit for total execution time in seconds"})
...
class SysbenchMemoryInputParams:
    ...
    time: typing.Optional[int] = field(default=10, metadata={
                                       "name": "Time", "description": "Limit for total execution time in seconds"})
    memoryblocksize: typing.Optional[str] = field(default='1KiB', metadata={
                                                  "name": "Block Size", "description": "size of memory block for test in KiB/MiB/GiB"})
    memorytotalsize: typing.Optional[str] = field(default='100G', metadata={
                                                  "name": "Total Size", "description": "Total size of data to transfer in GiB"})
    memoryscope: typing.Optional[str] = field(default='global', metadata={
                                              "name": "Memory Scope", "description": "Memory Access Scope(global/local)"})
    memoryoperation: typing.Optional[str] = field(default='write', metadata={
                                                  "name": "Memory Operation", "description": "Type of memory operation(write/read)"})
...
class LatencyAggregates:
    ...
    P95thpercentile: float = field(
        metadata={"name": "95th Percentile", "description": "95th percentile latency"})
    sum: float = field(
...
class ThreadFairnessAggregates:
    avg: float = field(
        metadata={"name": "Average", "description": "Average value across all threads"})
    stddev: float = field(metadata={
                          "name": "Standard Deviation", "description": "Standard deviation of all threads"})


@dataclass
class ThreadsFairness:
    events: ThreadFairnessAggregates = field(metadata={
                                             "name": "Thread Fairness events", "description": "number of events executed by the threads "})
    executiontime: ThreadFairnessAggregates = field(
        metadata={"name": "Thread Fairness execution time", "description": "Execution time of threads"})


@dataclass
class CPUmetrics:
    eventspersecond: float = field(metadata={
                                   "name": "Events per second", "description": "Number of events per second to measure CPU speed"})
...
class SysbenchMemoryOutputParams:
    ...
    totaltime: float = field(
        metadata={"name": "Total time", "description": "Total execution time of workload"})
    totalnumberofevents: float = field(metadata={
    ...
    operation: str = field(
        metadata={"name": "Operation", "description": "memory operation performed"})
    scope: str = field(
        metadata={"name": "Scope", "description": "scope of operation"})
    Totaloperationspersecond: float = field(metadata={
...
class SysbenchMemoryResultParams:
    ...
    transferred_MiB: float = field(
        metadata={"name": "Transferred memory", "description": "Total Memory Transferred"})
    transferred_MiBpersec: float = field(metadata={
                                         "name": "Transferred memory per second", "description": "Total Memory Transferred per second"})
    Latency: LatencyAggregates = field(
        metadata={"name": "Latency", "description": "Memory Latency in mili seconds"})
    Threadsfairness: ThreadsFairness = field(metadata={
                                             "name": "Threads fairness", "description": "Event distribution by threads for number of executed events by threads and total execution time by thread"})
...
class SysbenchCpuResultParams:
    ...
    CPUspeed: CPUmetrics = field(
        metadata={"name": "CPU speed", "description": "No of events per second"})
    Latency: LatencyAggregates = field(
        metadata={"name": "Latency", "description": "CPU latency in miliseconds"})
    Threadsfairness: ThreadsFairness = field(metadata={
                                             "name": "Threads fairness", "description": "Event distribution by threads for number of executed events by threads and total execution time by thread"})
...
def RunSysbenchCpu(params: SysbenchCpuInputParams) -> typing.Tuple[str, typing.Union[WorkloadResultsCpu, WorkloadError]]:
    ...
    try:
        cmd = ['sysbench', '--threads='+str(params.threads), '--events='+str(params.events), '--time='+str(
            params.time), '--cpu-max-prime='+str(params.cpumaxprime), params.operation, 'run']
...
def RunSysbenchMemory(params: SysbenchMemoryInputParams) -> typing.Tuple[str, typing.Union[WorkloadResultsMemory, WorkloadError]]:
    ...
    try:
        cmd = ['sysbench', '--threads='+str(params.threads), '--events='+str(params.events), '--time='+str(params.time), '--memory-block-size='+str(params.memoryblocksize),
               '--memory-total-size='+str(params.memorytotalsize), '--memory-scope='+str(params.memoryscope), '--memory-oper='+str(params.memoryoperation), params.operation, 'run']
        process_out = subprocess.check_output(cmd, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as error:
        return "error", WorkloadError(error.returncode, "{} failed with return code {}:\n{}".format(error.cmd[0], error.returncode, error.output))
